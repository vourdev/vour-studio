import { CONTACT_EMAIL, siteConfig } from "@/lib/site";
 ...
 * supported") as of this build, so shipping it would mean starting on an
 * unmaintained dependency for one transactional template. This produces the
 * same result with no dependency at all.
 *
 * Every interpolated value is visitor-supplied, so all of it is escaped.
 */
export function renderLeadNotification(lead: LeadEmailInput) {
  const rows: [string, string][] = [
    ...
    ["Tersimpan di database", lead.storedInDb ? "ya" : "tidak (Turso belum aktif)"],
  ];
...
<html lang="id">
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #e4e4e7;border-radius:12px;">
      <tr>
        <td style="padding:28px 28px 8px;">
          <p style="margin:0;font-size:12px;letter-spacing:0.14em;text-transform:uppercase;color:#71717a;">
            ${escapeHtml(siteConfig.name)} / lead baru
          </p>
      ...
      <tr>
        <td style="padding:20px 28px 28px;">
          ...
          <div style="white-space:pre-wrap;border-left:3px solid #c6f24e;padding:2px 0 2px 14px;font-size:14px;line-height:1.6;">${escapeHtml(lead.message)}</div>
          <p style="margin:24px 0 0;font-size:13px;">
            <a href="mailto:${escapeHtml(lead.email)}" style="color:#4d6b0a;">Balas ke ${escapeHtml(lead.email)}</a>
          </p>
        </td>
      </tr>
    </table>
  ...
  const text = [
    `Lead baru dari website ${siteConfig.name}`,
    "",
  ...
  return {
    ...
    text,
    replyTo: lead.email,
    to: process.env.LEAD_NOTIFICATION_EMAIL ?? CONTACT_EMAIL,
  };
}
